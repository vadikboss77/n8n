...
async function main() {
	...
	// ICP builder endpoint
	app.use('/icp', (await import('./routes/icp')).createIcpRouter(config));
	...
	app.post('/run', async (req, res) => {
		try {
			const date =
				typeof req.body?.date === 'string' ? req.body.date : new Date().toISOString().slice(0, 10);
